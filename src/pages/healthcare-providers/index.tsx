...
import { useRoqClient, useHealthcareProviderFindManyWithCount } from 'lib/roq';
...
export function HealthcareProviderListPage(props: HealthcareProviderListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'clinic', 'appointment.count'],
  ...
  const { data, error, isLoading, mutate } = useHealthcareProviderFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.healthcare_provider.delete({ where: { id } });
      await mutate();
    } catch (error) {
