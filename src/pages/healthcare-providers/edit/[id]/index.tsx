...
import { useRoqClient, useHealthcareProviderFindFirst } from 'lib/roq';
...
function HealthcareProviderEditPage() {
  ...
  const { data, error, isLoading, mutate } = useHealthcareProviderFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: HealthcareProviderInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.healthcare_provider.update({
        data: values as RoqTypes.healthcare_provider,
        where: {
          id,
        },
      });
