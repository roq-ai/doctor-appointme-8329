...
import { useRoqClient, usePatientFindManyWithCount } from 'lib/roq';
...
export function PatientListPage(props: PatientListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'insurance_provider', 'appointment.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['gender.contains', 'medical_history.contains'],
          ...params.filters,
        },
        'patient',
      ),
  ...
  const { data, error, isLoading, mutate } = usePatientFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.patient.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      header: 'Dob',
      accessorKey: 'dob',
      cell: ({ row: { original: record } }: any) => (record?.dob ? format(record?.dob, 'dd-MM-yyyy') : ''),
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/insurance-providers/view/${record.insurance_provider?.id}`}
            >
              {record.insurance_provider?.company_name}
            </Link>
          ),
        }
      : null,
