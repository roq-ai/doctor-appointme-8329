...
import { usePatientFindFirst, useRoqClient } from 'lib/roq';
...
function PatientViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; insurance_provider: boolean } }>(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'insurance_provider'],
          id,
        },
        'patient',
      ),
  ...
  const { data, error, isLoading } = usePatientFindFirst(queryParams, {}, { disabled: !id });
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem label="Dob" text={data?.dob ? format(data?.dob, 'dd-MM-yyyy') : ''} />
                  ...
                  <FormListItem
                    ...
                    text={
                      <Link as={NextLink} href={`/insurance-providers/view/${data?.insurance_provider?.id}`}>
                        {data?.insurance_provider?.company_name + ''}
                      </Link>
                    }
                  />
                )}
