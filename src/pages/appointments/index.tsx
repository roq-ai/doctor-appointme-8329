...
export function AppointmentListPage(props: AppointmentListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['patient', 'healthcare_provider', 'clinic'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['appointment_time.contains', 'reason_for_visit.contains'],
          ...params.filters,
        },
        'appointment',
      ),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'appointment_time', header: 'Appointment Time', accessorKey: 'appointment_time' },
    { id: 'reason_for_visit', header: 'Reason For Visit', accessorKey: 'reason_for_visit' },
    ...
    hasAccess('healthcare_provider', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'healthcare_provider',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/healthcare-providers/view/${record.healthcare_provider?.id}`}
            >
              {record.healthcare_provider?.specialization}
            </Link>
          ),
        }
      : null,
